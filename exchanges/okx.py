...
def okx_get_equity(ccy: str = OKX_MARGIN_CURRENCY) -> float:
    """Totale equity van de trading account in de opgegeven valuta (standaard USDC).

    Gebruikt 'eq' (totale equity, inclusief marge van open posities) zodat de
    positiegrootte altijd gebaseerd is op het volledige account — niet op het
    vrije saldo na aftrek van al geblokkeerde marge (dat zou 'availEq' zijn).
    Fallback naar cashBal → availEq als eq ontbreekt."""
    ...
    for d in data.get("details") or []:
        if str(d.get("ccy", "")).upper() == ccy.upper():
            for field in ("eq", "cashBal", "availEq"):
                value = d.get(field)
                if value not in (None, ""):
                    logger.info("OKX equity (%s) via veld '%s': %s", ccy, field, value)
                    return float(value)
